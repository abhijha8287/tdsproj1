...
function_definitions_llm = [
    {
        "name": "A1",
        "description": "Runs a Python script from a given URL, passing an email as the argument.",
        "parameters": {
            "type": "object",
            "properties": {
                "email": {"type": "string", "pattern": r"[\w\.-]+@[\w\.-]+\.\w+"},
                "url":{"type":"string" ,"pattern":r"https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)"}
            },
    ...
    {
        "name": "A8",
        "description": "Pass the image to an LLM, have it extract the card number, and write output without spaces",
        "parameters": {
            "type": "object",
            "properties": {
                "filename": {
...
async def read_file(path: str = Query(..., description="Filename to read")):
